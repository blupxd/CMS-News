  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-12">
        {posts.map((post) => (
          <div className="w-auto h-30 lg:col-span-2 md:col-span-4 sm:col-span-4 row-span-3 rounded-lg mt-4" key={post.title}>
            <div className="overflow-hidden">
                <a href={`/post/${post.slug}`}>
                    ...
                    alt={post.title}
                    href={`/post/${post.slug}`}
                    className="h-96 w-full md:h-64 object-cover"
                  />
                </a>
        ...
        {posts.map((post) => (
          <div className="w-auto h-30 lg:col-span-2 md:col-span-4 sm:col-span-4 row-span-1 rounded-lg mt-4" key={post.title}>
            <div className="flex border-l-4 border-red-500 pl-2 overflow-hidden">
              <div className="ml-2">
                    <h1 className="transition duration-300 w-72 truncate mb-0 cursor-pointer
                                hover:text-red-800 text-xl md:text-l font-semibold">
                            <Link href={`/post/${post.slug}`}>
                                {post.title}
                            </Link>
                    </h1>
                    <p className="w-72 h-12 truncate">{post.excerpt}</p>
                      <div className="inline-flex gap-1 items-center absolute mt-2">
                        <FontAwesomeIcon className="text-gray-500" icon={faClock} width="20px" height="20px" />
